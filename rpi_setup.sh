...
detect_and_configure_existing_images() {
    ...
    if [ "$images_found" = true ]; then
        ...
        echo -e "${BLUE}>>> All configurations documented in docker-compose.yml${NC}"
    else
...
add_mongodb_to_compose() {
  ...
  mongodb:
    ...
    sed -i '/^  # Services will be added here dynamically/a\'"$mongodb_service" "$COMPOSE_FILE"
    
    if ! grep -q "mongodb_data:" "$COMPOSE_FILE"; then
        sed -i '/^  # Named volumes will be added here dynamically/a\  mongodb_data:' "$COMPOSE_FILE"
    fi
}
...
add_mysql_to_compose() {
  ...
  mysql-db:
    ...
    sed -i '/^  # Services will be added here dynamically/a\'"$mysql_service" "$COMPOSE_FILE"
    
    if ! grep -q "mysql_data:" "$COMPOSE_FILE"; then
        sed -i '/^  # Named volumes will be added here dynamically/a\  mysql_data:' "$COMPOSE_FILE"
    fi
}
...
# =========================================
#
# This file contains all Docker services and their configurations.
# Service details and credentials are documented above each service.
#
# Usage:
#   - Start all services: docker-compose up -d
#   - Start specific service: docker-compose up -d <service-name>
...
services:
  # Services will be added here dynamically with their configurations
...
init_config_file() {
    # This function is now deprecated - configs go into docker-compose.yml
    echo -e "${YELLOW}>>> Configuration details are now saved in docker-compose.yml${NC}"
}
...
add_service_config() {
    ...
    # Check if service config already exists in docker-compose.yml
    if grep -q "# $service_name Configuration" "$COMPOSE_FILE"; then
        echo -e "${YELLOW}>>> Configuration for $service_name already exists in docker-compose.yml${NC}"
        return 0
    fi
    
    # Add service configuration as comments before the services section
    # Create temporary file with the new config
    local temp_file=$(mktemp)
    
    # Add config comments before services section
    sed '/^services:/i\
# =========================================\
...
#' "$COMPOSE_FILE" > "$temp_file"
    ...
    sudo chown "$ACTUAL_USER:$ACTUAL_USER" "$COMPOSE_FILE"
    echo -e "${GREEN}>>> Added $service_name configuration to docker-compose.yml${NC}"
}
...
show_menu() {
    ...
    if [ -f "$COMPOSE_FILE" ]; then
        ...
        echo "All configurations are documented in docker-compose.yml"
    fi
    echo "--------------------------------------------------"
}
...
install_all() {
    ...
    echo -e "All service configurations are documented in docker-compose.yml"
    echo ""
}
